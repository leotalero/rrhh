...
<script type="text/javascript">
	...
	$(document).ready(function() {	
		...
		$("#btnCrearActivo").button();
		$("#btnGuardarEntrevista").button();
		$("#fDesvinculacionActivos [name=btnGuardarEstadoproceso]").button();
		
		
		
		 $("#fDesvinculacionActivos").validate({
				...
				idestadoproceso: {
					required: "Seleccione una opción para el estado de proceso",
				},
				porcentajepuesto: {
...
function  cambioestado(idestadoproceso,idcontratoproceso,porcentaje){
	...
	if(idestadoproceso==1){//si es en proceso
		var porcentajepu=$("#fDesvinculacionActivos [name=porcentajepuesto]").val();
		
		
		fillporcentajeactivos(idcontratoproceso,porcentajepu);
	 	if(porcentajepu==100){
	 		
	 		$("#fDesvinculacionActivos [name=idestadoproceso]").val(2);
	 	}else{
	 	
	 		$("#fDesvinculacionActivos [name=idestadoproceso]").val(1);
	 	}
		
	}else if(idestadoproceso==2){//si es completado
		fillporcentajeactivos(idcontratoproceso,100);
		 $("#fDesvinculacionActivos [name=porcentajepuesto]").val(100);
	}
}
...
$("[name=lnkUpdatePropiedadesactivos]").click(function() {
...
<div align="left" id="dmDesvinculacionActivos">
<c:choose>
...
<div></div>
<form name="fDesvinculacionActivos" id="fDesvinculacionActivos" action="${ctx}/page/desvinculacion" method="post">
...
<div id="idTabsActivosPropiedades">
		<button type="button" id="btnCrearActivo" name="btnCrearActivo" onclick="Vercrearpropiedad(${contrato.idcontrato},${contratoproceso.proceso.idproceso});">Crear</button>&nbsp;&nbsp;&nbsp;
			
		<c:choose>
		
		<c:when test="${empty contratoprocesopropiedades}">
			<div class="msgInfo1" align="left">No cuenta con propiedades en Activos.</div>
			
		</c:when>
		<c:otherwise>
			
			
		<fieldset style="width: auto"><legend class="e6" >Activos</legend>	
		
				
				<span class="enlace" id='lnkUpdatePropiedadesactivos' name='lnkUpdatePropiedadesactivos'><a href="${ctx}/page/desvinculacion?action=${proceso.nombreproceso}&idcontrato=${contrato.idcontrato}&idproceso=${proceso.idproceso}"">&nbsp;Actualizar<span>&nbsp;</span></a></span>&nbsp;&nbsp;&nbsp;
				...
				<table style="font " border="0" width="100%" class="tExcel tRowSelect" >
						...
						<tbody>
								...
								<c:forEach items="${contratoprocesopropiedades}" var="activopropiedad" varStatus="loop">
								
								<tr style="color: ${activopropiedad.estado==2?'':'red'};">
								<td><c:out value="${activopropiedad.idcontratoprocesopropiedad}"/></td>
								<td class="e6" ><c:out value="${activopropiedad.procesopropiedad.nombrepropiedad}"/></td>
								
								<td><c:out value="${activopropiedad.dato}"/></td>
								<td><c:out value="${activopropiedad.observacion}"/></td>
								<td><c:out value="${activopropiedad.idusuariocrea}"/></td>
								<td><fmt:formatDate value="${activopropiedad.fechacrea}" pattern="dd/MM/yyyy H:mm"/></td>
								<td><c:out value="${activopropiedad.idusuariomod}"/></td>
								<td><fmt:formatDate value="${activopropiedad.fechamod}" pattern="dd/MM/yyyy H:mm"/></td>
								<td>
								<c:if test="${activopropiedad.estado==2}">
								<c:out value="Activo"/>
								</c:if>
								<c:if test="${activopropiedad.estado==3}">
							...
							<c:if test="${sessionScope[consHermes.USUARIO_SESSION].usuarioaplicacion.idgrupo == 1 || sessionScope[consHermes.USUARIO_SESSION].usuarioaplicacion.idgrupo == 214 || sessionScope[consHermes.USUARIO_SESSION].usuarioaplicacion.idgrupo == 215 || sessionScope[consHermes.USUARIO_SESSION].usuarioaplicacion.idgrupo == 216}">
									...
									<span class="enlace" onclick="deshabilitarContratoPropiedad(${activopropiedad.idcontratoprocesopropiedad},${contrato.idcontrato});" title="Deshabilitar"><img alt="Deshabilitar" src="${ctx}/imagen/control_deshabilitar.gif"></span>
									<span class="enlace" onclick="Vereditarpropiedad(${activopropiedad.idcontratoprocesopropiedad},${contrato.idcontrato},${contratoproceso.proceso.idproceso});" title="Editar"><img alt="Editar" src="${ctx}/imagen/control_editar.gif"></span>
									
									</c:if> 
									<c:if test="${activopropiedad.estado==3}">
									<span class="enlace" onclick="habilitarContratoPropiedad(${activopropiedad.idcontratoprocesopropiedad},${contrato.idcontrato});" title="Activar"><img alt="Activar" src="${ctx}/imagen/control_habilitar.gif"></span>
									</c:if>
							</c:if>	
			...
			<button type="submit" id="btnGuardarEstadoproceso" name="btnGuardarEstadoproceso">Guardar</button>&nbsp;&nbsp;&nbsp;
					...
					<table style="border: 2;float:none; ; width:100%" id="correspondencias" class="tExcel tRowSelect">
						 <col style="width: 50%;"/>		  
							<tr style="visibility: hidden" hidden="hidden">
									<th nowrap="nowrap" style="text-align: right;">Estado de Proceso:</th>
										...
										<script type="text/javascript">
										$("#fDesvinculacionActivos [name=idestadoproceso]").val("${contratoproceso.idprocesoestado}");
										</script>
							...
							<tr>
									<th nowrap="nowrap" style="text-align: right;">Estado de Proceso:</th>
										<td>
										<select name="porcentajepuesto" id="porcentajepuesto" onchange="cambioestado(1,${contratoproceso.idcontratoproceso},this.value)" >
										...
										<script type="text/javascript">
										$("#fDesvinculacionActivos [name=porcentajepuesto]").val("${contratoproceso.porcentaje}");
										</script>
