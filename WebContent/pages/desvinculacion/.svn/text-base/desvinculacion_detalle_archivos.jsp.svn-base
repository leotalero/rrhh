...
<script type="text/javascript">
	...
	$(document).ready(function() {	
		...
		$("#fDesvinculacionArchivos [name= btnCrearArchivo]").button();
		$("#btnGuardarEntrevista").button();
		$("#fDesvinculacionArchivos [name=btnGuardarEstadoproceso]").button();
		
		
		
		 $("#fDesvinculacionArchivos").validate({
...
function  cambioestado(idestadoproceso,idcontratoproceso,porcentaje){
	...
	if(idestadoproceso==1){//si es en proceso
		var porcentajepu=$("#fDesvinculacionArchivos [name=porcentajepuesto]").val();
		
		
		fillporcentajeactivos(idcontratoproceso,porcentajepu);
	 	if(porcentajepu==100){
	 		
	 		$("#fDesvinculacionArchivos [name=idestadoproceso]").val(2);
	 	}else{
	 	
	 		$("#fDesvinculacionArchivos [name=idestadoproceso]").val(1);
	 	}
		
	}else if(idestadoproceso==2){//si es completado
		fillporcentajeactivos(idcontratoproceso,100);
		 $("#fDesvinculacionArchivos [name=porcentajepuesto]").val(100);
	}
}
...
$("[name=lnkUpdatePropiedadesarchivos]").click(function() {
...
<div align="left" id="dmDesvinculacionArchivos">
<c:choose>
...
<div></div>
<form name="fDesvinculacionArchivos" id="fDesvinculacionArchivos" action="${ctx}/page/desvinculacion" method="post">
...
<div id="idTabsActivosPropiedades">
		<button type="button" id="btnCrearArchivo" name="btnCrearArchivo" onclick="Vercrearpropiedad(${contrato.idcontrato},${contratoproceso.proceso.idproceso});">Crear</button>&nbsp;&nbsp;&nbsp;
			
		<c:choose>
		
		<c:when test="${empty contratoprocesopropiedades}">
			<div class="msgInfo1" align="left">No cuenta con propiedades en Archivos.</div>
		...
		<fieldset style="width: auto"><legend class="e6" >${contratoproceso.proceso.nombreproceso}</legend>	
				...
				<span class="enlace" id='lnkUpdatePropiedadesarchivos' name='lnkUpdatePropiedadesarchivos'>
					<a href="${ctx}/page/desvinculacion?action=${proceso.nombreproceso}&idcontrato=${contrato.idcontrato}&idproceso=${proceso.idproceso}"">
					&nbsp;Actualizar
					<span>&nbsp;</span>
					</a>
				</span>&nbsp;&nbsp;&nbsp;
				...
				<table style="font " border="0" width="100%" class="tExcel tRowSelect" >
						...
						<tbody>
								...
								<td><c:out value="${propiedad.datoamostrar}"/></td>
							...
							<c:if test="${sessionScope[consHermes.USUARIO_SESSION].usuarioaplicacion.idgrupo == 1 || sessionScope[consHermes.USUARIO_SESSION].usuarioaplicacion.idgrupo == 214 || sessionScope[consHermes.USUARIO_SESSION].usuarioaplicacion.idgrupo == 215 || sessionScope[consHermes.USUARIO_SESSION].usuarioaplicacion.idgrupo == 216}">
									...
									<c:if test="${propiedad.estado==2}">
									<a href="${ctx}/page/desvinculacion?action=verArchivo&idcontratoprocesopropiedad=${propiedad.idcontratoprocesopropiedad}" 
					...
					<table style="border: 2;float:none; ; width:100%" id="correspondencias" class="tExcel tRowSelect">
							...
							<tr hidden="hidden">
									<th nowrap="nowrap" style="text-align: right;">Estado de Proceso:</th>
										...
										<script type="text/javascript">
										$("#fDesvinculacionArchivos [name=idestadoproceso]").val("${contratoproceso.idprocesoestado}");
										</script>
							...
							<tr>
									<th nowrap="nowrap" style="text-align: right;">Estado de Proceso:</th>
										...
										<script type="text/javascript">
										$("#fDesvinculacionArchivos [name=porcentajepuesto]").val("${contratoproceso.porcentaje}");
										</script>
